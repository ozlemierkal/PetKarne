...
/* v2.61 — approved onboarding behavior; deliberately shown on every page load */
(function(){
  function initPk61Onboarding(){
    const root = document.getElementById('pk61Onboarding');
    if(!root) return;

    const slides = Array.from(root.querySelectorAll('.pk61Slide'));
      ...
      if(openAddFriend){
        setTimeout(function(){
          const candidates = [
            ...
            document.querySelector('[data-action="add-pet"]'),
            document.querySelector('.add-pet-btn'),
    ...
    root.querySelectorAll('.pk61Skip').forEach(btn => {
      btn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation(); close(false);
      });
    });

    root.querySelectorAll('.pk61Next').forEach(btn => {
      btn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation(); show(index + 1);
      });
    });

    root.querySelectorAll('.pk61Dot').forEach(btn => {
      btn.addEventListener('click', function(e){
        e.preventDefault(); e.stopPropagation(); show(Number(btn.dataset.go));
      });
    });

    const add = document.getElementById('pk61AddFriend');
    if(add){
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', initPk61Onboarding, {once:true});
  } else {
    initPk61Onboarding();
  }
})();
