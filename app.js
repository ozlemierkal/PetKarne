...
/* v2.59 — gerçek onboarding davranışı */
(function(){
  const KEY='petkarnem_onboarding_v1_seen';

  function initPkOnboarding(){
    const root=document.getElementById('pkOnboarding');
    if(!root) return;
    ...
    document.body.classList.add('pkOnboardingOpen');
    const pages=[...root.querySelectorAll('.pkOnboardPage')];
    let page=0;
    ...
    function finish(openAddFriend){
      ...
      document.body.classList.remove('pkOnboardingOpen');
      root.remove();

      if(openAddFriend){
        // Use the EXISTING v2.58 add-friend flow; do not duplicate app logic.
        setTimeout(()=>{
          const addBtn=document.getElementById('addPetBtn');
          if(addBtn) addBtn.click();
        },0);
    ...
    root.querySelectorAll('.pkOnboardSkip').forEach(b=>{
    ...
    root.querySelectorAll('.pkOnboardDots button').forEach(b=>{
      b.addEventListener('click',(e)=>{
        e.preventDefault();
        e.stopPropagation();
        show(Number(b.dataset.goto||0));
      });
    });

    const add=root.querySelector('#pkOnboardAddFriend');
    if(add){
    ...
    root.addEventListener('touchend',(e)=>{
      ...
      if(dx<0 && page<pages.length-1) show(page+1);
      if(dx>0 && page>0) show(page-1);
    },{passive:true});
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',initPkOnboarding,{once:true});
  }else{
    initPkOnboarding();
  }
})();
